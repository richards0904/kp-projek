                                    ...
                                    <td>
                                        <div class="button-group">
                                            <button type="button" class="btn btn-success" data-bs-toggle="modal"
                                                data-bs-target="#konfirmasi{{ $pesanans->idPesanan }}" <?php if (($pesanans->status == 'Dikonfirmasi') || ($pesanans->status == 'Dibatalkan')){ ?> disabled
                                                <?php   } ?>>
                                                <i class="bi bi-check-square" style="margin-right: 5px"></i>
                                                Konfirmasi
                                            </button>
                                            <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#edit{{ $pesanans->idPesanan }}" data-idpesanan="{{ $pesanans->idPesanan }}" data-idtoko="{{ $pesanans->idToko }}">
                                                <i class="bi bi-pencil" style="margin-right: 5px"></i> Edit
                                            </button>
                                            <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                                data-bs-target="#delete{{ $pesanans->idPesanan }}">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form method="post" action="{{ route('hapus.pesanan.post')}}">
                                                    @csrf
                                                    <div class="modal-body">
                                                        <p style="margin-bottom: 5px">Apakah anda yakin ingin menghapus data pesanan ini? </p>
                                                        ...
                                                        <input type="hidden" name="hapusIdPesanan" value="{{ $pesanans->idPesanan }}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form autocomplete="off" action="{{route('tambah.pesanan.post')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
                            <select id="idToko" name="idToko" class="form-select" required>
                                <option value="" disabled selected>Pilih ID Toko</option>
                                @foreach ($tokoAll as $toko)
                                    <option value="{{ $toko->idToko }}" data-namatoko="{{ $toko->namaToko }}">{{ $toko->idToko }}</option>
                                @endforeach
                            </select>
                            <br>
